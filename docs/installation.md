...
Troubleshooting note: During testing with large RNA-seq FASTQ files (>15 GB), the USB 3.0 SSD connected directly to a Raspberry Pi 5 (i.e. not via externally powered USB hub) caused severe packet loss and SSH/Jupyter disconnections when operating over 2.4 GHz Wi-Fi. Connecting the SSD through an externally powered USB 3.0 hub completely resolved the issue while maintaining full USB 3.0 performance (~384 MB/s). Users experiencing Wi-Fi instability should consider using an externally powered USB hub, Ethernet networking, or 5 GHz Wi-Fi. Alternatively dont use the USB 3.0 hub, instead connect the USB SSD to the USB 2.0 hub on the Raspberry Pi 5.
