...
To see an example BAM file allignment to the mouse Leptin gene, see the screenshot located in the README.md file located in the jupyter folder. The screenshot shows BAM file allignment for files taken from a Raspberry Pi folder which is accessable publically. This is the most convenient method for visualising the alignment to a particular gene of interest.
